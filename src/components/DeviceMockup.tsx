...
const DeviceMockup = ({ 
  ...
  if (device === 'desktop') {
    ...
    // Much smaller desktop sizes
    const isLargeScreen = window.innerWidth >= 1024;
    const targetWidth = isLargeScreen ? 250 : 180; // Even smaller!
    ...
    return (
      <div className="relative animate-fade-in">
        {/* Desktop Frame */}
        <div 
          className="bg-gradient-to-b from-gray-200 to-gray-300 rounded-lg p-1"
          style={{ width: containerWidth + 8, height: containerHeight + 20 }}
        >
          {/* Screen Header Bar */}
          <div className="bg-gray-800 rounded-t-sm px-1 py-0.5 flex items-center justify-between mb-0.5">
          ...
          {/* Stand Base */}
          <div className="mt-0.5 mx-auto w-8 h-0.5 bg-gradient-to-r from-gray-400 to-gray-500 rounded-full"></div>
          <div className="mt-0.5 mx-auto w-10 h-0.5 bg-gradient-to-r from-gray-300 to-gray-400 rounded-full"></div>
        </div>
      </div>
    );
  }
  ...
  // Mobile sizes
  const isLargeScreen = window.innerWidth >= 1024;
  const targetWidth = isLargeScreen ? 120 : 70;
